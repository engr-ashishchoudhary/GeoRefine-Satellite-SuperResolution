"""
CLI entry point: run the full demo pipeline (Phases 5-8) for the demo
scene, then generate and validate demo_data/manifest.json.
...
# Allow running as `python scripts/run_full_pipeline.py` from repo root
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run GeoRefine's full demo pipeline (Phases 5-8).")
    ...
    print("Running full demo pipeline (inference, validation, uncertainty, NDVI)...")
    try:
        report = run_full_demo_pipeline(dataset_index_path, preprocessing_index_path, config, model)
    ...
    for key, relative_path in report["manifest"].items():
        print(f"  {key}: {relative_path}")
    print("Contract validation: all files present and openable.")
